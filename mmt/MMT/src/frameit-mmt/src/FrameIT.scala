  ...
  override val logPrefix = "frameit"
   ...
   private def GetResponse : HLet = new HLet {
     def act(tk : HTalk) = try {
       ...
       val vpathsS = tk.req.param("viewPath").getOrElse(throw FrameitError("no viewPath found"))
       ...
       val vpaths = vpathsS.split(" ").map{
         p => Path.parse(p) match {
           case vp : MPath => vp
           case p => throw FrameitError("Expected MPath found " + p)
         }
       }
       val tm = simplify(pushout(cpath, vpaths.toList))
       var tmS = tm.toString
   ...
   private def pushout(cpath : CPath, vpaths : List[MPath]) : Term = {
     println(controller.get(cpath.parent).toNode)
     ...
     val rulesList = vpaths.map(makeRules)
     log("Rules: " + rulesList)
     rulesList.foldLeft[Term](comp)((res,rules) => pushout(res)(rules))
   }
   
   
   def makeRules(vpath : MPath) : HashMap[Path, Term] = {
     ...
     v.from match {
       case OMMOD(p) =>
         ...
         v.components collect {
           case c : ConstantAssignment =>
             c.target.foreach {t =>
               println((p ? c.name).toString + " #->#" + t.toString)
               rules += (p ? c.name -> t)
             }
           case d : DefLinkAssignment => 
             rules ++= makeRules(d.target.toMPath)
         }
         rules
